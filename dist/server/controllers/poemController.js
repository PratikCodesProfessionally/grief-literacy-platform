...
class PoemController {
     ...
     * @param res Express Response
     */
    async listPoems(req, res) {
        try {
            const userId = req.user.id;
            const { page = 1, limit = 10, tags, search, sortBy = 'createdAt', order = 'desc' } = req.query;
            const filter = this.buildPoemFilters(userId, tags, search);
            // Pagination
    ...
    async updatePoem(req, res) {
        try {
            ...
            const updatedPoem = await Poem.findByIdAndUpdate(id, {
                ...updates,
                updatedAt: new Date()
            }, { new: true });
            res.json({
    ...
    // Hilfsmethoden
    validatePoemInput(title, content) {
    ...
    buildPoemFilters(userId, tags, search) {
